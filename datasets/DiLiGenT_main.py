...
class DiLiGenT_main(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 每次指定一个物体，处理这个物体的所有光度立体图像进行测试
        obj = self.objs[index]

        if obj == 'bearPNG':
            select_idx = np.random.permutation(np.arange(20, len(self.names)))[:self.args.in_img_num]
        else:
        ...
        if self.args.in_light:
            item['light'] = torch.from_numpy(self.l_dir[index][select_idx]).view(-1, 1,1).float()  
